...
import { COUNTRIES, CountryOption } from './data_countries';
...
const DisclaimerBanner = () => (
  <div className="bg-carbon-950 text-carbon-300 px-4 py-3 text-[10px] uppercase tracking-widest font-mono flex items-center justify-center border-b border-carbon-800 relative z-[60]">
    <ShieldAlert className="w-4 h-4 mr-3 text-signal-500" />
    <span>Strictly for Research Use Only. Not for human consumption.</span>
  </div>
);

const Footer = () => (
  <footer className="bg-white border-t border-carbon-200 pt-12 md:pt-20 pb-8 md:pb-12">
      ...
      <div className="max-w-screen-2xl mx-auto px-6 md:px-12 h-full">
        <div className="flex justify-between items-center h-full">
          ...
          <Link to="/" className="flex items-center space-x-3 group">
            <Hexagon className="w-6 h-6 text-carbon-900 stroke-1 group-hover:stroke-signal-600 transition-colors" />
          ...
          <div className="hidden md:flex items-center space-x-12">
            ...
            <Link to="/cart" className="flex items-center space-x-2 group">
              ...
              <div className="relative">
                ...
                {totalItems > 0 && (
                  <span className="absolute -top-2 -right-2 flex items-center justify-center h-4 w-4 text-[10px] font-bold text-white bg-signal-600 rounded-full">
          ...
          <div className="md:hidden flex items-center">
            <button onClick={() => setIsOpen(!isOpen)} className="text-carbon-900 p-2 -mr-2 hover:bg-carbon-50 rounded-sm transition-colors">
...
const CartPage = () => {
  ...
  const [selectedCountryData, setSelectedCountryData] = useState<CountryOption | null>(null);
  ...
  // Shipping warning logic: Close enough is defined as within $50 of the limit
  const remainingForFreeShipping = 300 - subtotal;
  const isCloseToFreeShipping = remainingForFreeShipping > 0 && remainingForFreeShipping <= 50;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const shippingLabel = shippingCost === 0 ? "FREE (Order > $300)" : `$${shippingCost.toFixed(2)}`;
...
_This request is for laboratory research use only._
      ...
      window.location.href = `https://wa.me/5522998004156?text=${encodedMessage}`;
      setIsSubmitting(false);
    }, 1500);
  };
              ...
              <div className="flex items-center space-x-8 mt-4 md:mt-0 w-full md:w-auto justify-between md:justify-end">
                ...
                <div className="flex items-center bg-carbon-50 border border-carbon-200 rounded-sm">
                   <button
                     ...
                     className="p-3 text-carbon-500 hover:text-carbon-900 hover:bg-carbon-200 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                     disabled={item.quantity <= 1}
                   >
                     <Minus className="w-4 h-4" />
                   </button>
                   <span className="font-mono text-sm text-carbon-900 w-10 text-center">{item.quantity}</span>
                   <button
                     onClick={() => updateQuantity(item.id, 1)}
                     className="p-3 text-carbon-500 hover:text-carbon-900 hover:bg-carbon-200 transition-colors"
                   >
          ...
          <div className="mt-8 pt-8 border-t border-carbon-900 animate-in fade-in duration-700 delay-300">
             ...
             {!isFreeShipping && (
                 <p className="text-xs text-right text-carbon-400 mt-2 font-mono">
                    Spend ${(300 - subtotal).toFixed(2)} more for free global shipping.
                 </p>
             )}
          </div>
        </div>
           ...
           {!showCheckout ? (
              <div className="bg-carbon-50 p-8 rounded-sm border border-carbon-200 sticky top-32 animate-in fade-in slide-in-from-right-4 duration-700">
                <p className="text-sm text-carbon-500 leading-relaxed mb-8">
                  By proceeding, you certify that these materials are for laboratory research use only and will not be used for any other purpose. Institutional email required.
                </p>
                <button 
              ...
              <form onSubmit={handleSubmit} className="bg-white p-8 md:p-10 border border-carbon-200 shadow-xl animate-in fade-in slide-in-from-bottom-4 duration-500 sticky top-32">
                 <h3 className="font-display text-2xl text-carbon-900 mb-8">Request Details</h3>
                 ...
                 <div className="space-y-5">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                       <input required name="firstName" placeholder="First Name" onChange={handleInputChange} className="w-full p-4 border border-carbon-200 text-base font-light text-carbon-900 bg-white placeholder:text-carbon-400 focus:border-carbon-900 outline-none rounded-sm transition-colors" />
                       <input required name="lastName" placeholder="Last Name" onChange={handleInputChange} className="w-full p-4 border border-carbon-200 text-base font-light text-carbon-900 bg-white placeholder:text-carbon-400 focus:border-carbon-900 outline-none rounded-sm transition-colors" />
                    </div>
                    <input required type="email" name="email" placeholder="Institutional Email" onChange={handleInputChange} className="w-full p-4 border border-carbon-200 text-base font-light text-carbon-900 bg-white placeholder:text-carbon-400 focus:border-carbon-900 outline-none rounded-sm transition-colors" />
                    <input required name="street" placeholder="Street Address" onChange={handleInputChange} className="w-full p-4 border border-carbon-200 text-base font-light text-carbon-900 bg-white placeholder:text-carbon-400 focus:border-carbon-900 outline-none rounded-sm transition-colors" />
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                       <div className="relative">
                           <select 
                                required 
                                ...
                                className="w-full p-4 pr-10 border border-carbon-200 text-base font-light text-carbon-900 bg-white focus:border-carbon-900 outline-none appearance-none rounded-sm transition-colors cursor-pointer"
                                defaultValue=""
                           >
                       ...
                       <input required name="zip" placeholder="Zip Code" onChange={handleInputChange} className="w-full p-4 border border-carbon-200 text-base font-light text-carbon-900 bg-white placeholder:text-carbon-400 focus:border-carbon-900 outline-none rounded-sm transition-colors" />
                    </div>
                 </div>
...
export default function App() {
  ...
  const showToast = (message: string) => {
    ...
    // 2. Schedule exit animation trigger (e.g., after 3 seconds)
    setTimeout(() => {
        setToastState(prev => prev.message ? { ...prev, isExiting: true } : prev);
        
        // 3. Remove from DOM after exit animation completes (500ms matches CSS transition)
        setTimeout(() => {
